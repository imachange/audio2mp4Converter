...
### 主要な機能
- 音声ファイルと画像ファイルの自動検出（`audio.*` / `cover.*` を候補として検出し、許可拡張子のみ処理）
- 画像サイズの自動調整（奇数サイズ対応）
...
このリポジトリには GitHub Actions を用いた自動テストが設定されています（`.github/workflows/test_audio2mp4.yml`）。
ワークフローでは、Windows / macOS / Linux の 3OS 上で FFmpeg をセットアップし、ダミーの音声・画像ファイルを生成してスクリプトを実行し、`movie.mp4` が生成されることを確認します。

自動テストに加えて、以下の点を手動で確認してください：

1. 各プラットフォームでの実環境に近い条件での動作確認（Windows, macOS, Linux）
2. 様々な音声・画像形式での動作確認
